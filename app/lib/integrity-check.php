...
function check_system_integrity()
{
    ...
    if (version_compare(PHP_VERSION, '5.6.0', '<')) {
        $install_errors[] = 'This server is currently running PHP version ' . PHP_VERSION . ' and katchy needs at least PHP 5.6.0 to run. You need to update PHP in this server.';
    }
    if (ini_get('allow_url_fopen') !== 1 && !function_exists('curl_init')) {
        $install_errors[] = "cURL isn't installed and allow_url_fopen is disabled. katchy needs one of these to perform HTTP requests to remote servers.";
    }
    ...
    if (preg_match('/apache/i', $_SERVER['SERVER_SOFTWARE']) && function_exists('apache_get_modules') && !in_array('mod_rewrite', apache_get_modules())) {
        $install_errors[] = 'Apache <a href="http://httpd.apache.org/docs/2.1/rewrite/rewrite_intro.html" target="_blank">mod_rewrite</a> is not enabled in this server. This must be enabled to run katchy.';
    }
        ...
        if (!file_exists($v)) { // Exists?
            ...
            if (!is_writable($v)) {
                ...
                if (!$handle || fwrite($handle, 'Test write operation') === false) {
                    $install_errors[] = 'No write permission for PHP user ' . get_current_user() . ' in <code>' . G\absolute_to_relative($v) . '</code> directory. katchy needs to be able to write in this directory.';
                } else {
                    fclose($handle);
                    unlink($testFile);
                }
    ...
    if (is_array($install_errors) && count($install_errors) > 0) {
        ...
        Render\katchy_die($install_errors);
    }
}
