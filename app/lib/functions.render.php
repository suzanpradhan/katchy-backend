...
{
    echo    '<meta name="generator" content="' . G_APP_NAME . ' ' . CHV\get_katchy_version() . '">' . "\n" .
...
function include_peafowl_foot()
{
    ...
    $resources = [
        ...
        'katchy' => G_APP_PATH_LIB . 'katchy.js',
    ];
    ...
    $echo = [
        ...
        '<script defer data-cfasync="false" src="' . $resources['katchy'] . '" id="katchy-js"></script>',
    ];
    ...
    if (method_exists('CHV\Settings', 'getkatchy')) {
        $echo[] = '<script data-cfasync="false">var katchy = ' . json_encode(CHV\Settings::getkatchy()) . '</script>';
    }
    echo implode("\n", $echo);
}
...
function katchy_die($error_msg, $paragraph = null, $title = null)
{
    ...
    if (is_null($paragraph)) {
        $paragraph = "The system has encountered errors that must be fixed to allow proper katchy functionality. katchy won't work until the following issues are solved:";
    }
    $solution = "Need help or questions about this? Go to <a href='http://katchy.com/support' target='_blank'>katchy support<a/>.";
    $title = (!is_null($title)) ? $title : 'System error';
    $doctitle = $title . ' - katchy';
...
{
    return $src . '?' . md5(CHV\get_katchy_version());
}
