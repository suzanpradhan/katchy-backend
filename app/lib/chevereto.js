...
$(function () {
  ...
  $(document).on(
    "keyup change",
    "[data-action=resize-combo-input]",
    function (e) {
  ...
  $(document).on(
    "click",
    "[data-action=list-tools] [data-action]",
    function (e) {
      var $this = $(e.target),
            ...
            case "image":
              ...
              $("[name=form-album-id]", modal_source)
                .find(
                  "[value=" +
                    $list_item.data(
                      dealing_with == "image" ? "album-id" : "id"
                    ) +
                    "]"
                )
                .attr("selected", true);
          ...
          PF.fn.modal.call({
            ...
            confirm: function () {
              ...
              if (PF.obj.modal.form_data.editing.new_album) {
                ...
                if (
                  PF.obj.modal.form_data.editing.album_privacy == "password"
                ) {
  ...
  $(document).on(
    "change",
    "[data-content=user-avatar-upload-input]",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      if (
        user_avatar_file.size >
        CHV.obj.config.user.avatar_max_filesize.getBytes()
      ) {
        PF.fn.growl.call(
          PF.fn._s(
  ...
  $(document).on(
    "contextmenu",
    CHV.fn.listingViewer.selectors.src,
    function (e) {
      e.preventDefault();
      return false;
    }
...
CHV.fn.listingViewer = {
  ...
  filler: function (isOpened) {
    ...
    $.each($tools.find("[data-action]"), function (i, v) {
      ...
      $(
        '<div class="viewer-kb-key" data-key="' +
          keymap[0] +
          '"><kbd>' +
          keymap[0] +
          "</kbd><span>" +
    ...
    if ($(results.success, this.selectors.root).is(":visible")) {
      ...
      if (typeof this.results.success[firstKey].image.album !== "undefined") {
        ...
        if (albums.length <= 1) {
          ...
          targetAlbum.link =
            this.results.success[firstKey].image.user.url_albums;
