...
class Settings
{
    ...
    public function __construct()
    {
        try {
            ...
            // katchy demo only
            if (!in_array($_SERVER['SERVER_NAME'], ['demo.katchy.com'])) {
                if ($settings['twitter_account'] == 'katchy') {
    ...
    public static function getkatchy()
    {
        $api = 'https://katchy.com/api/';
        include_once G_APP_PATH . 'license/key.php';
        $id = explode(':', $license)[0];
        $info = $api . 'get/info/';
        if (!defined('G_APP_GITHUB_REPO_URL')) {
            $info .= '?id=' . $id;
            $label = 'katchy.com/panel/downloads';
            $url = 'https://katchy.com/panel/downloads';
        } else {
            $info .= 'free';
