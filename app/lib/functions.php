 ...
 * ----------------------------------------------------------------------------------------------------------------------------------------.
 */
function get_katchy_version($full = true)
{
...
function getSystemNotices()
{
    ...
    if (getSetting('update_check_display_notification') && (version_compare(getSetting('update_check_notified_release'), getSetting('katchy_version_installed'), '>') && version_compare(getSetting('update_check_notified_release'), G_APP_VERSION, '>'))) {
    ...
    if (version_compare(G_APP_VERSION, getSetting('katchy_version_installed'), '>')) {
    ...
    if (!in_array($_SERVER['SERVER_ADDR'], ['127.0.0.1', '::1']) && !in_array($_SERVER['SERVER_NAME'], ['demo.katchy.com'])) {
        ...
        if (preg_match('/@katchy\.com$/', getSetting('email_from_email')) || preg_match('/@katchy\.com$/', getSetting('email_incoming_email'))) {
...
function katchyID($in, $action = 'encode')
{
    global $katchyID;
...
// Shorthand for katchyID encode
function encodeID($var)
{
    return katchyID($var, 'encode');
}

// Shorthand for katchyID decode
function decodeID($var)
{
    return katchyID($var, 'decode');
}
...
function checkUpdates()
{
    try {
        ...
        $katchy = Settings::getkatchy();
        $update = G\fetch_url($katchy['api']['get']['info']);
        if (isSafeToExecute() && $update) {
            ...
            if (is_null(getSetting('update_check_notified_release')) || (version_compare($latest_release, G_APP_VERSION, '>') && version_compare($latest_release, getSetting('update_check_notified_release'), '>'))) {
                // Email notify
                $message = _s('There is an update available for your katchy based website.') . ' ' . _s('The release notes for this update are:');
                $message .= "\n\n";
                ...
                $message .= _s('You can apply this update directly from your %a or download it from %s and then manually install it.', ['%a' => '<a href="' . G\get_base_url('dashboard?checkUpdates') . '" target="_blank">' . _s('admin dashboard') . '</a>', '%s' => '<a href="' . $katchy['source']['url'] . '" target="_blank">' . $katchy['source']['label'] . '</a>']) . "\n\n";
                $message .= '--' . "\n" . 'katchy' . "\n" . G\get_base_url();
                $message = nl2br($message);
                system_notification_email(['subject' => sprintf('katchy update available (v%s)', $latest_release), $latest_release, 'message' => $message]);
                $settings_update = [
