...
{
	$message = _s("This functionality is not part of katchy Free. %s to obtain this feature.", ['%s' => '🚀<a data-action="upgrade">' . _s('Upgrade') . '</a>']);
					...
					<ul class="tabbed-content-list table-li margin-top-20">
						<li>
							...
							<span class="display-table-cell vertical-align-middle" style="line-height: 1;">
								<a class="github-button" href="https://github.com/katchy/katchy-free/subscription" data-icon="octicon-eye" data-size="large" data-show-count="true" aria-label="Watch Ckatchy/katchy-free on GitHub">Watch</a>
								<a class="github-button" href="https://github.com/katchy/katchy-free" data-icon="octicon-star" data-size="large" data-show-count="true" aria-label="Star katchy/katchy-free on GitHub">Star</a>
							</span>
						</li>
						<?php
	...
	<form id="dashboard-settings" method="post" data-type="<?php echo get_dashboard(); ?>" data-action="validate" enctype="multipart/form-data">
		...
		?>
			...
			<?php if (get_settings()['key'] == 'website') { ?>
				...
				<div class-="phablet-c1">
					<div class="input-label">
						<label for="enable_powered_by"><?php _se('Powered by'); ?> katchy</label>
						...
						<div class="input-below"><?php _se('Enable this if you want to show a "%s" link at the footer.', _s('Powered by') . ' katchy'); ?></div>
					</div>
				</div>
			...
			<?php
				...
				?>
					...
					<div class="input-label">
						...
						<div class="input-below"><?php _se('Check the <a %s>icon reference</a> for the complete list of supported icons.', 'href="http://katchy.com/src/icomoon" target="_blank"'); ?></div>
					</div>
			...
			<?php if (get_settings()['key'] == 'image-upload') { ?>
				...
				<div class="input-label">
					...
					<div class="c9 phablet-c1"><input type="text" name="upload_image_path" id="upload_image_path" class="text-input" value="<?php echo get_safe_post() ? get_safe_post()['upload_image_path'] : CHV\Settings::get('upload_image_path'); ?>" placeholder="<?php _se('Relative to katchy root'); ?>" required></div>
					<span class="input-warning red-warning"><?php echo get_input_errors()['upload_image_path']; ?></span>
					<div class="input-below"><?php _se('Where to store the images? Relative to katchy root.'); ?></div>
				</div>
				<div class="input-label">
			...
			?>
				...
				<div class="c24 phablet-c1 highlight text-content padding-10 margin-bottom-0">
					...
					<p>Both language and username subdomains are highly experimental features that require advanced server configurations. The alleged functionalities should be enabled only after setting up a subdomain wildcard pointing to your katchy installation. You also need to enable CORS for these subdomains. If you want to use HTTPS, you also need to bind the SSL certificate(s) for your subdomains.</p>
