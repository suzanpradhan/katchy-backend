...
if (!is_readable(dirname(__FILE__) . '/settings.php')) {
    if (!@fopen(dirname(__FILE__) . '/settings.php', 'w')) {
        die("katchy can't create the app/settings.php file. You must manually create this file.");
    }
}
...
// Set the URLs
define('G_ROOT_URL', G_HTTP_PROTOCOL . '://' . G_HTTP_HOST . G_ROOT_PATH_RELATIVE); // http(s)://www.mysite.com/katchy/
define('G_ROOT_LIB_URL', G\absolute_to_url(G_ROOT_LIB_PATH)); // not used
define('G_APP_LIB_URL', G\absolute_to_url(G_APP_PATH_LIB));

// Folders definitions
define('CHV_FOLDER_IMAGES', !is_null(Settings::get('katchy_version_installed')) ? Settings::get('upload_image_path') : 'images');
...
if (!defined('G_APP_PATH_THEME')) {
    ...
    if (Settings::get('katchy_version_installed')) {
...
if (access !== 'install' and Settings::get('katchy_version_installed')) {
